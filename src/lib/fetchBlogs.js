...
export async function fetchBlogs() {
  try {
    ...
    const seenTitles = new Set(blogData.map((b) => b.title.toLowerCase()));
    ...
    // ✅ Return local blogData posts (newest/latest added first) plus API posts
    const sortedLocal = [...blogData].sort((a, b) => b.id - a.id);
    return [...sortedLocal, ...filtered];
  } catch (err) {
    console.error(err);
    return [...blogData].sort((a, b) => b.id - a.id);
  }
}
